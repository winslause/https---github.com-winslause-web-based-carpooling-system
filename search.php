        ...
        <div class="col-md-9 col-md-push-3">
          <div class="result-sorting-wrapper">
            <div class="sorting-count">
              ...
              $sql = "SELECT tblvehicles.id from tblvehicles join tblroutes on tblroutes.id=tblvehicles.routename where tblvehicles.VehicleTitle=:search || tblvehicles.picktime=:search || tblroutes.routeName=:search || tblvehicles.freeseats=:search";
          ...
          <?php
          $sql = "SELECT tblvehicles.*,tblroutes.routeName,tblroutes.id as bid  from tblvehicles join tblroutes on tblroutes.id=tblvehicles.routename where tblvehicles.VehicleTitle=:search || tblvehicles.picktime=:search || tblroutes.routeName=:search || tblvehicles.freeseats=:search";
          $query = $dbh->prepare($sql);
          $query->bindParam(':search', $searchdata, PDO::PARAM_STR);
          $query->execute();
          $results = $query->fetchAll(PDO::FETCH_OBJ);
              ...
              <!-- <form action="search.php" method="post">
                <div class="form-group select">
                  <select name="searchdata" class=" form-control">
                    <option name="searchdata">Select Route</option>
                    ...
                    if ($query->rowCount() > 0) {
                      foreach ($results as $result) {       ?>
                        <option name="searchdata" value=" <?php echo htmlentities($result->id); ?>"><?php echo htmlentities($result->routeName); ?></option>
                    <?php }
                    } ?>
                ...
                <div class="form-group">
                  <!-- <button type="submit" name="searchdata" class="btn btn-block"><i class="fa fa-search" aria-hidden="true"></i> Search Car</button> -->
            </div>
            </form>
          </div>
      </div>
